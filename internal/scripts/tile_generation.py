...
raw_images_path = r'\\PNL\Projects\SLIC-TCF\Summer 2023 Interns\Intern Collaborative Workspace\Machine Learning Testing\Data\Big 500'
raw_tiles_path =  r'\\PNL\Projects\SLIC-TCF\Summer 2023 Interns\Intern Collaborative Workspace\Machine Learning Testing\Data\Big 500'
...
#Loop/open the images and divide them into tiles and put them in the "tiles raw" folders
raw_images_dir = Path(raw_images_path).glob('*.tif') # ??? Set file type here. Define the list of images to loop through
for pic in raw_images_dir: #Loop through the photos
    name = str(pic) 
    ...
    folder_path = os.path.join(raw_tiles_path, name_only) # Creates the full folder path name for the individual coupon name at this step in the loop.
